...
import secrets
import os
from flask import Blueprint, current_app, render_template, flash, redirect, url_for, request, g
...
def before_product_request():
    ...
    if g.delete.validate_on_submit():
...
def home():
    ...
    if current_user.is_authenticated:
        ...
        return render_template('home.html', image_file=image_file, product=products, notifs=notifs)
...
def add_to_wishlists(id):
    productz = Products.query.get_or_404(id)
    add_to_wishlist(id)
    notif_mssg = (productz.product_name + ' has been added to your wishlist')
...
def register():
    if current_user.is_authenticated:
        ...
        if form.validate_on_submit():
            ...
            user = User(name=(first_name + " " + last_name ), email=email, username=username, password=hashed_password)
            db.session.add(user)
            db.session.commit()
            login_user(user)
            ...
            db.session.add(new_notifs)
            db.session.commit()
            flash('Account has been created '+ first_name + " " + last_name + '. Complete your Profile')
    ...
    if current_user.is_authenticated:
        ...
        if form.validate_on_submit():
            if form.product_img.data: 
                pics = []  
                for pictures in form.product_img.data:   
                    img_file  =  save_product_pic(pictures)
                    pics.append(img_file)
                print(pics)
            product_data = {
